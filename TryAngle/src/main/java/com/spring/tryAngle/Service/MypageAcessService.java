package com.spring.tryAngle.Service;

import java.util.Map;

import org.springframework.ui.Model;
import org.springframework.web.servlet.ModelAndView;

import com.spring.tryAngle.DAO.MemberDAO;
import com.spring.tryAngle.DTO.MemberDTO;
import com.spring.tryAngle.Util.Construct;

public class MypageAcessService implements MyService{

	@Override
	public void excute(ModelAndView model) {
		// TODO Auto-generated method stub
		
	}

	@Override
	public void excute(Model model) {
		// TODO Auto-generated method stub
		
	}

	@Override
	public boolean isExcute(ModelAndView model) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public boolean isExcute(Model model) {
		// TODO Auto-generated method stub
		boolean sw = false;
		
		try {

			Map<String, Object> map = (Map) model.asMap();
			
			String pw = (String) map.get("pw"); // 패스워드
			
			String str = (String) map.get("str"); // 아이디
			
			pw = Construct.getIntence().getPassword(pw);
			
			MemberDTO dto= Construct.getIntence().getSql().getMapper(MemberDAO.class).LoginOk(str, pw);
			
			if(dto.getId()!=null) {
				
				sw = true;
				
			}
			
		}catch(Exception e) {
			e.printStackTrace();
		}
		
		return sw;
	}

}
